...
public class VideoController {
      int importacion;
        ...
        private ImportacionesViewDAO vistaImportacion;
          ...
          @RequestMapping("/multimedia/importacionesV")
	public ModelAndView ListImportacionesVeh(HttpSession session){
           ...
           if(u.getIdrol()==3){
            ...
            return new ModelAndView("multimedia/video/busquedaImportacion").addObject("usuario",u).addObject("importaciones",list);
           }
           else{
    ...
    @RequestMapping("/videomultimedia/new/{idvehiculo}")
	public ModelAndView multimediaImagenes(@PathVariable int idvehiculo, HttpSession session) {
          ...
          return new ModelAndView("/multimedia/video/agregarVideo").addObject("usuario",u).addObject("multimediaveh",multimedia);
        }
...
FileInputStream fis = null;
  if (!file.isEmpty()) {
   try {
    ...
    stream.close();
       ...
       tm.setIdtipomultimedia(3);// es video
  ...
  return "redirect:/multimedia/video/"+ve.getIdvehiculo();
 }

  @RequestMapping("/multimedia/video/{idvehiculo}")
	public String ListImagenes(@PathVariable int idvehiculo, HttpSession session,Model model){
            ...
            return "multimedia/video/mostrarVideo";
    }
        
     
    
         @RequestMapping("/multimediaVeh/findV/{idimport}")
	public ModelAndView vehiculosImportacionMult(@PathVariable int idimport, HttpSession session) {
             ...
             importacion=idimport;
              ...
              // List<Vehiculos> veview = vistaVehiculo.findByidimport(idimport);
               return new ModelAndView("/multimedia/video/busquedaVehiculo").addObject("usuario",u).addObject("vehiculo",vehiculos).addObject("idimport",idimport);
	}
