...
public class VideoIController {
    ...
    private ImportacionesViewDAO vistaImportacion;
     ...
     public Importador obtenerImportadorByUsuario(Usuario u){
        ...
        @RequestMapping("/imagenVeh/importacionesV")
	public ModelAndView ListImportacionesVeh(HttpSession session){
           ...
           if(u.getIdrol()==3){
            ...
            return new ModelAndView("imagenveh/video/busquedaVehImportacion").addObject("usuario",u).addObject("importaciones",list);
    }
           else{
         ...
         @RequestMapping("/imagenVeh/findV/{idimport}")
	public ModelAndView vehiculosImportacionImg(@PathVariable int idimport, HttpSession session) {
              ...
              // List<Vehiculos> veview = vistaVehiculo.findByidimport(idimport);
               return new ModelAndView("/imagenVeh/video/busquedaVehiculos").addObject("usuario",u).addObject("vehiculo",vehiculos).addObject("idimport",idimport);
	}
        
        @RequestMapping("/imagenveh/agregarV/{idvehiculo}")
	public ModelAndView multimediaImagenes(@PathVariable int idvehiculo, HttpSession session) {
          ...
          return new ModelAndView("/imagenveh/video/agregarVideo").addObject("usuario",u).addObject("imagenveh",multimedia);
        }
        
        
          //===============================Video=================================================
//=======================fUNCIONA CON UNA IMAGEN================================================
@RequestMapping(value = "/uploadVentaVideo", method=RequestMethod.POST)
...
FileInputStream fis = null;
  if (!file.isEmpty()) {
   try {
    ...
    stream.close();
       ...
       tm.setIdtipomultimedia(3);// es audio
  ...
  return "redirect:/imagenveh/video/"+ve.getIdvehiculo();
 }

  @RequestMapping("/imagenveh/video/{idvehiculo}")
	public String ListImagenes(@PathVariable int idvehiculo, HttpSession session,Model model){
            ...
            return "imagenveh/video/mostrarVideo";
    }
